...
Every model here is on a free tier, so the invoice is $0 — which is exactly why
the number worth showing is what the run *would* cost at list price. Zero teaches
nobody anything; "$0.027 for one plan" is the figure that decides whether the
design survives a paid account.

The plan page shows it per run, next to calls, tokens and wall clock. Two things
make it trustworthy rather than decorative:

**The rates are not hand-typed.** `providers/prices.py` is generated by
`make prices` from `litellm.model_cost` — the same map the proxy bills against
and Langfuse costs traces with. A hand-maintained table drifts silently: the
number keeps rendering, it is just wrong.

**The model being priced is the one that answered.** The app asks the proxy for
`planner`, an alias over three deployments, and the response body echoes that
alias back. Pricing the alias reports every proxied run as unpriced; pricing it
as zero reports every run as free. The deployment that actually served the call
is in `x-litellm-model-name`, and LiteLLM's own figure for that call is in
`x-litellm-response-cost` — so the header cost is used when present, and the
local table only prices direct, unproxied calls.

An unknown model is `None`, never `0.0`, and unpriced calls are counted
separately so the total reads as a floor instead of a lie.

Checked against Langfuse for the same run: per-agent figures match to the cent —
`flight $0.011834`, `hotels $0.007415`, `weather $0.006863`, `places $0.000213`.
The one difference was instructive: Langfuse priced `budget` at $0, because those
two generations reached it labelled with the alias, which its price map cannot
resolve. Reading the served deployment from the header is what closes that gap.
...
providers/         LLM routing, cache, checkpointing, metrics, tracing,
                   prompt registry, untrusted text, list-price costing
